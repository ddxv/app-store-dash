<script lang="ts">
	...
	function typeTabClass(tab: string) {
		const selectedClass =
			'px-4 py-2 border-2 border-b-0 border-primary-100 bg-white rounded-t-md relative top-[1px]';
		const unselectedClass =
			'px-4 py-2 border-b-2 border-transparent border-surface-100 hover:border-primary-300 hover:text-gray-700';
		if (tab === 'all') {
