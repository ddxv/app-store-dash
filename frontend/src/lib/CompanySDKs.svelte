<script lang="ts">
	...
	export let title = 'Company SDKs'; // Default title, can be overridden

	function truncateList(list: string[], maxItems = 3) {
		return list.slice(0, maxItems);
	}

	// Get unique paths across all companies
	const uniquePaths = [...new Set(Object.values(mySdks.companies).flatMap((c) => c.paths))];
</script>
