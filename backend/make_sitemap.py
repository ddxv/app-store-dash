...
def set_df_sitemap_columns(df: pd.DataFrame, priority: float, changefreq: str = 'weekly') -> pd.DataFrame:
...
def create_paginated_sitemaps(
    ...
    Args:
        ...
        base_url: Base URL for sitemap entries
...
# about 6
company_types = cdf.groupby("type_url_slug")['app_count'].sum().sort_values(ascending=False).reset_index()
company_types['url'] = 'https://appgoblin.info/companies/types/' + company_types['type_url_slug']
company_types = set_df_sitemap_columns(company_types, 0.9)

# about 50
company_categories = cdf.groupby("app_category")['app_count'].sum().sort_values(ascending=False).reset_index()
company_categories['url'] = 'https://appgoblin.info/companies/categories/' + company_categories['app_category']
company_categories = set_df_sitemap_columns(company_categories, 0.8)

# about 5 * 50
company_type_categories = cdf.groupby(["type_url_slug", "app_category"])['app_count'].sum().sort_values(ascending=False).reset_index()
company_type_categories['url'] = 'https://appgoblin.info/companies/types/' + company_type_categories['type_url_slug'] + '/' + company_type_categories['app_category']
company_type_categories = set_df_sitemap_columns(company_type_categories, 0.7)


# about 1700
companies = cdf.groupby("company_domain")['app_count'].sum().sort_values(ascending=False).reset_index()
companies['url'] = 'https://appgoblin.info/companies/' + companies['company_domain']
companies = set_df_sitemap_columns(companies, 0.8)


# about 40k
company_domain_categories = cdf.groupby(["company_domain", "app_category"])['app_count'].sum().sort_values(ascending=False).reset_index()
# about 28k
company_domain_categories[company_domain_categories['app_count'] > 20]
company_domain_categories['url'] = 'https://appgoblin.info/companies/' + company_domain_categories['company_domain'] + '/' + company_domain_categories['app_category']
...
# Larger
paginated_sitemaps = create_paginated_sitemaps(company_domain_categories, 'sitemap_company_domain_categories', 'https://appgoblin.info/companies/')
paginated_sitemaps += create_paginated_sitemaps(apps, 'sitemap_apps', 'https://appgoblin.info/apps/')
